...
public class UserServiceBean implements UserService {
  ...
  public ResponseObject createUser(User user) {
    ...
    if (userFromDb.isEmpty()) {
      ...
      emailService.emailConfirmation(
          new EmailDetails(
              user.getEmail(),
              "Welcome to LeisureLink app "
                  + savedUserDetails.getName()
                  + " "
                  + savedUserDetails.getLastName(),
              "Email confirmation"));
  ...
  public ResponseObject signin(User user) {
    ...
    try {
      ...
      return new ResponseObject(
          HttpStatus.ACCEPTED, "CORRECT_LOGIN_DATA", token, userLoginData.get().getAppUserRoles());
    } catch (Exception e) {
  ...
  public ResponseObject passwordResetting(String code, String password) {
    log.debug(
        "TutorService ==> passwordResetting() - start:  code = {}, password = {}", code, password);
  ...
  public ResponseObject resetPassword(User user, String password) {
    ...
    userRepository.save(user);
    return new ResponseObject(
        HttpStatus.ACCEPTED, "PASSWORD_SUCCESSFULLY_UPDATED", refresh(user.getEmail()));
  }
  ...
  public ResponseObject updateUserDetails(UpdateUserDetailsDTO updateUserDetailsDTO, UUID userId) {
    ...
    if (user.getUserDetails() != null) userDetails.setId(user.getUserDetails().getId());
  ...
  public String refresh(String email) {
    ...
    if (tutor.isEmpty()) {
      ...
      return jwtTokenProvider.createToken(
          email, Collections.singletonList(tutor.get().getAppUserRoles()));
    }
  }
