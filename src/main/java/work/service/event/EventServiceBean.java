...
public class EventServiceBean implements EventService {
  ...
  public ResponseObject createEvent(HttpServletRequest request, EventCreateDto eventToCreate) {
    ...
    if (eventToCreate.photos() != null) {
      List<CompletableFuture<EventImage>> futures =
          eventToCreate.photos().stream()
              .map(
                  photo ->
                              ...
                              () -> {
                                try {
                                  ...
                                  throw new CustomException(
                                      "WRONG_IMAGE_FORMAT", HttpStatus.BAD_REQUEST);
                                }
                              })
                          .handle(
                              (result, ex) -> {
      ...
      Set<EventImage> eventImages =
          futures.stream().map(CompletableFuture::join).collect(Collectors.toSet());
    ...
    if (event.getAddress() == null || event.getAddress().isEmpty()) {
      geodataService
          .getAddressFromCoordinates(event.getLocation().getY(), event.getLocation().getX())
          .subscribe(
              addressJson -> {
  ...
  public ResponseObject createEventComment(
    ...
    return new ResponseObject(
        HttpStatus.CREATED, "COMMENT_CREATED", authenticationService.extractRequestToken(request));
    //        } catch (Exception e) {
    //            var event = eventRepository.findById(eventId).orElseThrow(() -> new
    // CustomException("EVENT_NOT_FOUND", HttpStatus.NOT_FOUND));
    //            var comment = commentMapper.fromCreateCommentDto(createCommentDto);
    //            comment.setEvent(event);
    //            comment.setCommentDate(ZonedDateTime.now());
  ...
  public List<EventsInRadiusDto> getEventsWithinRadius(
    ...
    if (searchEventDTO.startDate() != null) {
      events =
          eventRepository.findEventsWithinRadius(
              searchEventDTO.latitude(),
              searchEventDTO.longitude(),
              searchEventDTO.radius(),
              searchEventDTO.startDate());
    } else {
      events =
          eventRepository.findEventsWithinRadius(
              searchEventDTO.latitude(), searchEventDTO.longitude(), searchEventDTO.radius());
    }
    if (searchEventDTO.selectedCategories() != null
        && !searchEventDTO.selectedCategories().isEmpty()) {
    ...
    if (StringUtils.isNotBlank(searchEventDTO.eventName())) {
      events =
          events.stream().filter(e -> e.getName().contains(searchEventDTO.eventName())).toList();
    }

    var response = events.stream().map(eventMapper::eventToEventsInRadiusDto).toList();
    List<Event> finalEvents = events;

    //        response.forEach(r -> {
    //            List<CompletableFuture<Void>> futures = r.getEventImages().stream()
    //                    .map(p -> CompletableFuture.runAsync(() ->
    // p.setImage(utilService.decompressImage(p.getImage()))))
    //                    .toList();
    //
    //            CompletableFuture.allOf(futures.toArray(new CompletableFuture[0])).join();
    //        });
        ...
        r -> {
          for (var event : finalEvents) {
            if (event.getId().equals(r.getId())) {
              r.setNumberOfMembers(
                  event.getMembers().stream()
                      .filter(member -> !member.getStatus().equals(AppMemberStatus.STATUS_INACTIVE))
                      .toList()
                      .size());
              ...
              var host =
                  event.getMembers().stream()
                      .filter(f -> f.getType().equals(AppMemberType.ROLE_HOST))
                      .findFirst()
                      .orElseThrow()
                      .getUser();
              r.setHost(
                  new Host(
                      host.getId(),
                      host.getUserDetails().getName(),
                      host.getUserDetails().getLastName()));
            }
          }
        });
  ...
  public CertainEventDto getCertainEvent(UUID id) {
                    ...
                    CompletableFuture.runAsync(
                        () -> {
                          byte[] decompressedData =
                              utilService.decompressImage(eventImage.getImage());
    ...
    var responseHost =
        new Host(host.getUser().getId(), userDetails.getName(), userDetails.getLastName());
    // TODO
  ...
  public List<MembersForUserDto> getMembersForCertainEvent(UUID eventId) {
    ...
    var members =
        event.getMembers().stream()
            .filter(
                member ->
                    member.getType() != AppMemberType.ROLE_HOST
                        && member.getStatus() != AppMemberStatus.STATUS_INACTIVE)
            .toList();
  ...
  public ResponseObject removeCurrentUserFromEvent(HttpServletRequest request, UUID eventId) {
    ...
    var event =
        eventRepository
            .findEventByIdAndUserId(user.getId(), eventId)
            .orElseThrow(() -> new CustomException("EVENT_NOT_FOUND", HttpStatus.NOT_FOUND));
    event.getMembers().stream()
    ...
    eventRepository.saveAndFlush(event);
    return new ResponseObject(
        HttpStatus.OK, "SUCCESSFULLY", authenticationService.extractRequestToken(request));
  }
  ...
  public List<EventDto> getEventsWithPagination(
    ...
    if (searchEventDTO.startDate() != null) {
      ...
      eventPage =
          eventRepository.findEventsWithinRadiusWithPagination(
              searchEventDTO.latitude(),
              searchEventDTO.longitude(),
              searchEventDTO.radius(),
              searchEventDTO.startDate(),
    ...
    if (searchEventDTO.selectedCategories() != null
      ...
      if (result != null && StringUtils.isNotBlank(searchEventDTO.eventName())) {
        result =
            result.stream().filter(e -> e.getName().contains(searchEventDTO.eventName())).toList();
      }
      result =
          eventPage.stream()
              .filter(
      ...
      response = eventPage.stream().map(eventMapper::eventToEventDto).collect(Collectors.toList());
            ...
            for (var event : finalEventPage) {
              if (event.getId().equals(r.getId())) {
                r.setNumberOfMembers(
                    event.getMembers().stream()
                        .filter(
                            member -> !member.getStatus().equals(AppMemberStatus.STATUS_INACTIVE))
                        .toList()
                        .size());
                var categories =
                    event.getCategories().stream().map(EventCategory::getName).toList();
                r.setCategories(categories);
                var host =
                    event.getMembers().stream()
                        .filter(f -> f.getType().equals(AppMemberType.ROLE_HOST))
                        ...
                        .orElseThrow();
                var userDetails =
                    userDetailsRepository.findByUserId(host.getUser().getId()).orElseThrow();
                var responseHost =
                    new Host(host.getId(), userDetails.getName(), userDetails.getLastName());
                r.setHost(
                    new Host(responseHost.id(), responseHost.name(), responseHost.lastname()));
              }
            }
          });
      return getEventDtos(response);
    ...
    finalEventPage = eventPage;
    response = result.stream().map(eventMapper::eventToEventDto).toList();

    response.forEach(
        r -> {
          List<CompletableFuture<Void>> futures =
              r.getEventImages().stream()
                  .map(
                      p ->
                          CompletableFuture.runAsync(
                              () -> p.setImage(utilService.decompressImage(p.getImage()))))
