...
public class UserControllerBean implements UserController {
  ...
  public ResponseObject confirmPasswordResetting(PasswordResetDTO passwordResetDTO) {
    return userService.passwordResetting(
        passwordResetDTO.getCode(), passwordResetDTO.getPassword());
  }
  ...
  @Override
  public ResponseObject updateUserDetails(
      HttpServletRequest request, UpdateUserDetailsDTO detailsDTO) {
    return userService.updateUserDetails(
        detailsDTO, authenticationService.getUserByToken(request).getId());
  }

  @Override
  public ResponseObject updateUserImage(HttpServletRequest request, MultipartFile photo) {
    return userService.updateUserImage(
        authenticationService.getUserByToken(request).getId(), photo);
  }
